 ...
 * @author luke
 */
public class WorldGauntlet extends World {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private static final float BOUNDRY_WIDTH = 40000;
	...
	private String[][] TEXTS = { { "AFTER THE LONG JOURNEY TO", "ANDROMEDA, THE NACHT HAVE PLACED", "MANY OBSTACLES AS A DETERRENT.", "",
			"NOT ENOUGH FOR THE SPACEINATOR!" } };
	...
	public void start(GameState gameState) {
		...
		if (centreSpaceShip == null) {
			...
			pb.addPoint(-BOUNDRY_WIDTH + w * 8, h * .6168f);
			pb.addPoint(-BOUNDRY_WIDTH + w * 8, -h * .6168f);
			pb.addPoint(-BOUNDRY_WIDTH + w * 1.8f, -h);
			pb.move();
			pb.addPoint(BOUNDRY_WIDTH - w * 8, -h);
			pb.addPoint(BOUNDRY_WIDTH - w * 8.2f, h);
			pb.addPoint(BOUNDRY_WIDTH - w * 2, h * .6168f);
			pb.addPoint(BOUNDRY_WIDTH - w * 2, -h * .6168f);
			pb.addPoint(BOUNDRY_WIDTH - w * 8, -h);
	...
	public int getLeaderBoardID() {
		return R.string.leaderboard_gauntlet;
	}
}
